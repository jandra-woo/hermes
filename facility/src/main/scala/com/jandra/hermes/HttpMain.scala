...
import akka.actor.{ActorRef, ActorSystem, AddressFromURIString, RootActorPath}
...
//  }
  ...
  class WebServer extends JsonSupport {
    val config = ConfigFactory.load("src/main/resources/application_http.conf")
      ...
      path("order") {
        post {
          entity(as[OrderRequest]) { orderrequest =>
            complete {
              (cClient ? ClusterClient.Send("/user/mastersharding", orderrequest, localAffinity = true)).mapTo[JobResult]
            }
          }
        }
      }
